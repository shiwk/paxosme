...
namespace paxosme {
    ...
    std::shared_ptr<NetworkClient> NetworkClient::New(const Peer &peer){
        auto client = GrpcClient::NewClient(peer.ToString());
        return std::shared_ptr<NetworkClient>(client);
    }
}
